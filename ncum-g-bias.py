...
mean_ncum_g_2 = ncum_g_2_regridded['APCP_surface'].mean(dim='time')
...
mean_ncum_g_2 = mean_ncum_g_2.rio.write_crs("EPSG:4326").rio.clip(shape.geometry.apply(mapping), shape.crs)
...
bias_ncum_g_2.plot(ax=ax2, cmap=cmap, norm=norm, cbar_kwargs={'label': 'Bias (mm)', 'shrink': 0.5})
shape.boundary.plot(ax=ax2, edgecolor='black', linewidth=0.7, transform=ccrs.PlateCarree())
ax2.add_feature(cfeature.COASTLINE, linewidth=0.5)
ax2.set_title('Mean Rainfall Bias NCUM-G_day_2', fontsize=12)

ax3 = plt.subplot(1, 3, 3, projection=ccrs.PlateCarree())
bias_ncum_g_3.plot(ax=ax3, cmap=cmap, norm=norm, cbar_kwargs={'label': 'Bias (mm)', 'shrink': 0.5})
shape.boundary.plot(ax=ax3, edgecolor='black', linewidth=0.7, transform=ccrs.PlateCarree())
ax3.add_feature(cfeature.COASTLINE, linewidth=0.5)
ax3.set_title('Mean Rainfall Bias NCUM-G_day_3', fontsize=12)
