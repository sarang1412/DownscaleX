...
ncum_g_day_2 = ncum_g_2_regridded.sel(time=t, method='nearest')
...
ncum_g_1_event = ncum_g_1_event.sel(lat=slice(6, 41), lon=slice(65, 106))
ncum_g_2_event = ncum_g_2_event.sel(lat=slice(6, 41), lon=slice(65, 106))
ncum_g_3_event = ncum_g_3_event.sel(lat=slice(6, 41), lon=slice(65, 106))
...
ncum_g_1_event['APCP_surface'].plot(ax=ax2, cmap='Blues', transform=ccrs.PlateCarree(),cbar_kwargs={'label': 'rainfall(mm/day)','shrink': 0.9}, vmin=0,vmax=100
)
ax2.add_feature(cfeature.COASTLINE, linewidth=0.5)
shape.boundary.plot(ax=ax2, edgecolor='black', linewidth=0.7, transform=ccrs.PlateCarree())
ax2.set_title('Event Rainfall - NCUM-G Day 1', fontsize=12)

# Plot for NCUM-G Day 2
ax3 = plt.subplot(2, 2, 3, projection=ccrs.PlateCarree())
ncum_g_2_event['APCP_surface'].plot(ax=ax3, cmap='Blues', transform=ccrs.PlateCarree(),cbar_kwargs={'label': 'rainfall(mm/day)','shrink': 0.9}, vmin=0,vmax=100
)
ax3.add_feature(cfeature.COASTLINE, linewidth=0.5)
shape.boundary.plot(ax=ax3, edgecolor='black', linewidth=0.7, transform=ccrs.PlateCarree())
ax3.set_title('Event Rainfall - NCUM-G Day 2', fontsize=12)

# Plot for NCUM-G Day 3
ax4 = plt.subplot(2, 2, 4, projection=ccrs.PlateCarree())
ncum_g_3_event['APCP_surface'].plot(ax=ax4, cmap='Blues', transform=ccrs.PlateCarree(),cbar_kwargs={'label': 'rainfall(mm/day)','shrink': 0.9}, vmin=0,vmax=100
)
...
ax4.set_title('Event Rainfall - NCUM-G Day 3', fontsize=12)
