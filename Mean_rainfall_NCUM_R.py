...
ncum_r_2_mean = ncum_r_2_regridded.mean(dim='time')
...
ncum_r_1_mean = ncum_r_1_mean.rio.write_crs("EPSG:4326").rio.clip(shape.geometry.apply(mapping), shape.crs)
ncum_r_2_mean = ncum_r_2_mean.rio.write_crs("EPSG:4326").rio.clip(shape.geometry.apply(mapping), shape.crs)
ncum_r_3_mean = ncum_r_3_mean.rio.write_crs("EPSG:4326").rio.clip(shape.geometry.apply(mapping), shape.crs)
...
obs_mean['rf'].plot(ax=ax1, cmap=hexa, transform=ccrs.PlateCarree(),cbar_kwargs={'label': 'mean rainfall(mm/day)','shrink': 0.9}, vmin=0,vmax=52
)
...
ncum_r_1_mean['APCP_surface'].plot(ax=ax2, cmap=hexa, transform=ccrs.PlateCarree(),cbar_kwargs={'label': 'mean rainfall(mm/day)','shrink': 0.9}, vmin=0,vmax=52
)
ax2.add_feature(cfeature.COASTLINE, linewidth=0.5)
shape.boundary.plot(ax=ax2, edgecolor='black', linewidth=0.7, transform=ccrs.PlateCarree())
ax2.set_title('Mean Rainfall NCUM-R Day 1', fontsize=12)

# Plot for NCUM-G Day 2 Mean Rainfall
ax3 = plt.subplot(2, 2, 3, projection=ccrs.PlateCarree())
ncum_r_2_mean['APCP_surface'].plot(ax=ax3, cmap=hexa, transform=ccrs.PlateCarree(),cbar_kwargs={'label': 'mean rainfall(mm/day)','shrink': 0.9}, vmin=0,vmax=52
)
ax3.add_feature(cfeature.COASTLINE, linewidth=0.5)
shape.boundary.plot(ax=ax3, edgecolor='black', linewidth=0.7, transform=ccrs.PlateCarree())
ax3.set_title('Mean Rainfall NCUM-R Day 2', fontsize=12)

# Plot for NCUM-G Day 3 Mean Rainfall
ax4 = plt.subplot(2, 2, 4, projection=ccrs.PlateCarree())
ncum_r_3_mean['APCP_surface'].plot(ax=ax4, cmap=hexa, transform=ccrs.PlateCarree(),cbar_kwargs={'label': 'mean rainfall(mm/day)','shrink': 0.9}, vmin=0,vmax=52
)
...
ax4.set_title('Mean Rainfall NCUM-R Day 3', fontsize=12)
